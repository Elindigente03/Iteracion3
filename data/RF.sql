...
select USUARIO.NOMBRE, USUARIO.ID , SERVICIO.NOMBRE ,servicio.id, count(servicioid)   
from ((usuario iner join afiliado on afiliado.id = usuario.id) inerr join Cita on cita.IDUSUARIO)  
WHERE count(servicioid) >12 


select servicioid   
from  Cita on cita.IDUSUARIO)  
WHERE asistio = 1 and count(to_char(fecha,'MM/YY'))>3 
Group by to_char(fecha,'MM/YY')

 CREATE TABLE CITAJOINAFILIADO
(
  IDCITA NUMBER,
  IDAFILIADO NUMBER,
  CONSTRAINT CITA PRIMARY KEY ((IDCITA,IDAFILIADO)
);

ALTER TABLE CITA
ADD CONSTRAINT IDCITAFK
FOREIGN KEY (IDCITA)
REFERENCES CITA (ID)
ENABLE;

ALTER TABLE CITA
ADD CONSTRAINT IDAFILIADOFK
